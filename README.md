...
4. **Отслеживание эволюции базы (Дрифт)**
...
4. L4 получает доменный профиль из классификации текста, а L3/L2 собирают микс из дочерних узлов. Если знак модуля расходится с полученной информацией - сервер сообщает о дрифте.
...
| `index_all` | Переиндексировать всю базу; с `with_embeddings=true` также обновляет эмбеддинги файлов/чанков |
| `embed` | Получить вектор для текста |
| `chunk_text` | Разрезать Markdown-текст на стабильные чанки |
| `chunk_file` | Разрезать тело одной заметки на стабильные чанки |
| `search_chunks` | Искать по сохранённым чанкам; по умолчанию снижает анизотропию |
